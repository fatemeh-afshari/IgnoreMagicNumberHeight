 ...
 * </compliant>
 */
class CustomMagicNumber(config: Config) : Rule(
    config
) {
    ...
    override val issue: Issue
        get() = Issue(javaClass.simpleName, Severity.Style, "custom magic number", Debt.FIVE_MINS)
    ...
    private fun isIgnoredByConfig(expression: KtConstantExpression) = when {
        ...
        expression.name?.contains("height" , ignoreCase = true)==true ->true
        else -> false
    }
    ...
    private fun parseAsDouble(rawNumber: String): Double {
        ...
        return when {
            normalizedText.startsWith("0x") -> normalizedText.substring(2).toLong(HEX_RADIX)
                .toDouble()

            normalizedText.startsWith("0b") -> normalizedText.substring(2).toLong(BINARY_RADIX)
                .toDouble()
    ...
    private fun KtConstantExpression.isCompanionObjectProperty() =
        isProperty() && isInCompanionObject()
